...
check_path_safety() {
  ...
  # Check for path traversal attempts (just warn, don't fail)
  if [[ "$path" == *".."* ]]; then
    log_warn "Path contains '..': $path"
  fi

  # Basic validation - path should start with / or a letter
  if [[ ! "$path" =~ ^[/a-zA-Z~] ]]; then
    log_warn "Unusual path format: $path"
  fi
...
main() {
  ...
  case "$target" in
    ...
    user)
      ...
      ;;
    *)
      die "$EXIT_GENERAL_ERROR" "Unknown target: $target"
      ;;
  esac

  local avail_path="$avail_dir/$CONFIG_NAME"
  local link_path="$conf_dir/$CONFIG_NAME"
  local lock_file="/tmp/${SCRIPT_NAME}.${target}.lock"

  # Permission checks (except for status mode)
  if [[ "$mode" != "status" ]]; then
    ...
    acquire_lock "$lock_file" "$dry_run"
  ...
  case "$mode" in
    install)
      ...
      validate_directory_writable "$avail_dir" "$dry_run" || \
        die "$EXIT_PERMISSION_DENIED" "Cannot write to $avail_dir"
      validate_directory_writable "$conf_dir" "$dry_run" || \
        die "$EXIT_PERMISSION_DENIED" "Cannot write to $conf_dir"

      # Check and install fonts if needed
      ...
      if [[ $refresh -eq 1 ]]; then
        refresh_cache "$dry_run" "$target"
      else
        log "Skipping fc-cache refresh (--no-cache specified)"
      fi

      # Apply Flatpak override if requested
      ...
      if [[ $dry_run -eq 0 ]]; then
        ...
        log "The Noto fonts are now prioritized for sans-serif, serif, and monospace."
      else
        echo
    ...
    status)
      report_status "$avail_path" "$link_path" "$target"
      ;;

    *)
      die "$EXIT_GENERAL_ERROR" "Unsupported mode: $mode"
      ;;
  esac
